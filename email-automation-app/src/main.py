...
class EmailAutomationApp(tk.Tk):
    ...
    def init_messages_tab(self):
        ...
        self.campaign_listbox.bind(
            "<<ListboxSelect>>", self.on_campaign_select_messages
        )
        ...
        ttk.Button(add_camp_frame, text="Dodaj", command=self.add_campaign).pack(
            side="left"
        )
        ...
        ttk.Button(steps_btn_frame, text="Dodaj krok", command=self.add_step).pack(
            side="left", padx=2
        )
        ttk.Button(steps_btn_frame, text="Usuń krok", command=self.remove_step).pack(
            side="left", padx=2
        )
        ...
        self.attachments_listbox = tk.Listbox(
            self.attachments_section, width=50, height=8
        )
        ...
        self.add_attachment_btn = ttk.Button(
            btns_frame, text="Dodaj załącznik", command=self.add_attachment
        )
        self.add_attachment_btn.pack(side="left", padx=2)
        self.remove_attachment_btn = ttk.Button(
            btns_frame, text="Usuń załącznik", command=self.remove_attachment
        )
        self.remove_attachment_btn.pack(side="left", padx=2)
        self.save_btn = ttk.Button(
            self.template_form_frame, text="Zapisz", command=self.save_current_step
        )
        self.disable_template_form()

        # Komunikaty
        self.message_status_label = ttk.Label(
            self.tab_messages, text="", foreground="green"
        )
    ...
    def load_attachments(self):
        ...
        for att in attachments:
            ...
            self.current_attachments.append(
                {"id": att_id, "filename": filename, "remote_path": remote_path}
            )
    ...
    def add_attachment(self):
        ...
        try:
            ...
            self.attachments_manager.add_attachment(
                self.current_step_id, filename, remote_path
            )
    ...
    def init_analytics_tab(self):
        ...
        self.analytics_campaign_listbox.bind(
            "<<ListboxSelect>>", self.on_analytics_campaign_select
        )
        ...
        self.send_button = ttk.Button(
            right_frame,
            text="Uruchom wysyłkę",
            command=self.run_campaign_send,
            state="disabled",
        )
        self.send_button.pack(pady=5)
        # Przycisk sprawdzania odpowiedzi
        self.check_resp_button = ttk.Button(
            right_frame,
            text="Sprawdź odpowiedzi",
            command=self.check_responses,
            state="disabled",
        )
    ...
    def load_campaign_progress(self):
        ...
        if not self.selected_analytics_campaign_id:
            return
        rows = self.analytics_progress_manager.get_progress_for_campaign(
        ...
        for _id, email, company, stage, last_send, response in rows:
            self.progress_tree.insert(
                ...
                values=(
                    ...
                    company,
                    stage,
                    str(last_send) if last_send else "",
        ...
        for idx, contact in enumerate(contacts):
            ...
            if current_step_idx == 0:
                ...
                if stage == "not_sent":
                    try:
                        self.send_status_label.config(
                            text=f"Wysyłam do: {email} ({idx+1}/{len(contacts)})"
                        )
                        ...
                        server.sendmail(
                            EMAIL_CONFIG["from_address"], email, msg.as_string()
                        )
                        server.quit()
                ...
                if last_send is not None and days_after_prev is not None:
                    last_send_dt = (
                        last_send
                        if isinstance(last_send, datetime.datetime)
                        else datetime.datetime.strptime(
                            str(last_send), "%Y-%m-%d %H:%M:%S"
                        )
                    )
                    if (datetime.datetime.now() - last_send_dt).days >= days_after_prev:
                        try:
                            self.send_status_label.config(
                                text=f"Wysyłam do: {email} ({idx+1}/{len(contacts)}) - {name}"
                            )
                            self.update_idletasks()
                            msg = MIMEMultipart()
                            msg["From"] = EMAIL_CONFIG["from_address"]
                            ...
                            msg["Subject"] = subject
                            msg.attach(MIMEText(body, "plain"))
                            attachments = self.attachments_manager.get_attachments(
                                step_id
                            )
                            for att in attachments:
                                _, filename, remote_path = att
    ...
    def check_responses(self):
        ...
        if not self.selected_analytics_campaign_id:
            ...
            logging.error("Nie wybrano kampanii do sprawdzenia odpowiedzi.")
            return
        ...
        existing_contact_ids = set([c[0] for c in all_contacts])
        ...
        if not imap_host or not imap_user or not imap_pass:
            messagebox.showerror(
                "Błąd IMAP",
                "Brak danych logowania do serwera IMAP w pliku .env lub config.py!",
            )
            logging.error("Brak danych logowania do IMAP.")
            return
        try:
            with IMAPClient(imap_host, ssl=True) as server:
                server.login(imap_user, imap_pass)
                server.select_folder("INBOX")
                # Szukaj ostatnich 100 wiadomości (nie tylko UNSEEN)
                messages = server.search(["ALL"])
                last_100 = messages[-100:] if len(messages) > 100 else messages
                found_any = False
                for uid, message_data in server.fetch(last_100, ["ENVELOPE"]).items():
                    envelope = message_data[b"ENVELOPE"]
                    # Obsłuż wiele nadawców (np. reply-all)
                    from_addrs = []
                    if envelope.from_:
                    ...
                    for from_addr in from_addrs:
                        if from_addr in email_to_contact:
                            ...
                            if contact_id not in existing_contact_ids:
                                logging.warning(
                                    f"Pominięto aktualizację: contact_id={contact_id}, email={from_addr} nie istnieje w email_addresses."
                                )
                                continue
                            logging.info(
                                f"Próba aktualizacji statusu na 'responded' dla contact_id={contact_id}, email={from_addr}"
                            )
                            try:
                                self.analytics_progress_manager.add_or_update_progress(
                                    self.selected_analytics_campaign_id,
                                ...
                                logging.error(
                                    f"Błąd SQL przy aktualizacji contact_id={contact_id}, email={from_addr}: {sql_ex}"
                                )
                self.load_campaign_progress()
                if found_any:
                    messagebox.showinfo(
                        "Odpowiedzi",
                        "Znaleziono odpowiedzi w skrzynce odbiorczej i zaktualizowano statusy!",
                    )
                else:
                    messagebox.showinfo(
                        "Odpowiedzi",
                        "Nie znaleziono odpowiedzi od kontaktów kampanii w ostatnich 100 wiadomościach.",
                    )
        except Exception as e:
            logging.error(f"Błąd ogólny IMAP: {e}")
            messagebox.showerror(
                "Błąd IMAP", f"Nie udało się połączyć z serwerem IMAP:\n{e}"
            )
        ...
        if stage == "responded":
            ...
            with IMAPClient(imap_host, ssl=True) as server:
                ...
                if messages:
                    ...
                    msg_data = server.fetch([latest_uid], ["RFC822"])[latest_uid][
                        b"RFC822"
                    ]
                    ...
                    if msg.is_multipart():
                        for part in msg.walk():
                            if part.get_content_type() == "text/plain":
                                msg_text = part.get_payload(decode=True).decode(
                                    errors="ignore"
                                )
                                break
                    else:
            ...
            if stage == "welcome_sent":
                ...
                tpl = templates_manager.get_template(
                    "reminder", self.selected_analytics_campaign_id
                )
            elif stage == "last_offer_sent":
                tpl = templates_manager.get_template(
                    "last_offer", self.selected_analytics_campaign_id
                )
            if tpl:
                subject, body, *_ = tpl
    ...
    def show_message_status(self, text, color="green"):
        ...
        self.message_status_label.after(
            4000, lambda: self.message_status_label.config(text="")
        )
